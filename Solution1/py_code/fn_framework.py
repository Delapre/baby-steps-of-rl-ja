    ...
    def policy(self, s):
        if np.random.random() < self.epsilon or not self.initialized:
            ...
            with open('log.txt', 'a') as f:
                print("policy_actions:",self.actions,"   policy_action",action,file=f)
...
class Trainer():
    ...
    def train_loop(self, env, agent, episode=200, initial_count=-1,
        ...
        for i in range(episode):
            ...
            with open('log.txt', 'a') as f:
                print("train_loop_done:",done,file=f)
            ...
            while not done:
                ...
                if self.training and observe_interval > 0 and\
                   (self.training_count == 1 or
                    ...
                    frames.append(s)
                with open('log.txt', 'a') as f:
                    print("train_loop_state:",s,file=f)

                a = agent.policy(s)
                n_state, reward, done, info = env.step(a)
                with open('log.txt', 'a') as f:
                    print("train_loop_a:",a,"   train_loop_n_state:",n_state,"   train_loop_reward",reward,file=f)
